$(document).ready(function () {
    CallCKEditor('.post-classic-editor');
    const statusBullet = document.getElementById("new-post-status-bullet"), statusClasses = ["bg-success", "bg-warning", "bg-danger", "bg-primary"], datePicker = document.getElementById("new-post-datepicker"), statusSelect = document.getElementById("Status");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == "success") {
                    $('input[name="ThumbPath"]').val(response.url);
    ...
    $('#LanguagesList').on('change', function () {
        if ($(this).val() != $(this).data('default')) {
            $('.translation-card .card-footer').html(`<span class="text-danger">حتما مقاله اصلی را انتخاب کنید</span>`);
            $('#OriginalArticle .select2').removeClass('d-none');
            if ($('#OriginalArticle').val() == null || $('#OriginalArticle').val() == '')
                $('#add_product_submit').addClass('d-none');
        }
        else {
            $('.translation-card .card-footer').html('');
            $('#add_product_submit').removeClass('d-none');
            $('#OriginalArticle .select2').addClass('d-none');
        }
    });

    $('#OriginalArticle').on('change', function () {
        if ($(this).val() != null)
            $('#add_product_submit').removeClass('d-none');
    });
